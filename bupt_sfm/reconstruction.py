...
def incremental_reconstruction(cloud_viewer: CloudView, cfg: Dict[str, Any]) -> Tuple[np.ndarray, np.ndarray]:
    ...
    print("Initializing...")
    if cfg["running_robust"]:
        image_id1, image_id2, cloud, colors = Initializer.Initialize(cfg, cloud_viewer)
    else:
        image_id1, image_id2, cloud, colors = Initializer.FastInitialize(cfg, cloud_viewer)

    print("Initialization finished, incremental reconstructing...")
    ...
    while len(co_features_graph.get_edges()) > 0:
        ...
        if cfg["BA_activated"]:
            cameraArray = np.hstack([rvecs.reshape(1, 3), t.reshape(1, 3),
                                     np.array([incre_cv_node.intrinsics[0, 0], 0., 0.], dtype=np.float32).reshape(1, 3)])
            points3D = cloud
